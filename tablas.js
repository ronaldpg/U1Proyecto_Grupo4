...
// Custom Element: <runaway-tabla></runaway-tabla>

class RunawayTabla extends HTMLElement {
    connectedCallback() {
        const horariosData = [
        ...
        const filasHTML = horariosData.map(horario => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <button class="tabla-btn-action btn-ver-detalle" data-id="${horario.id}">
                            Ver
                        </button>
                        <button class="tabla-btn-action btn-eliminar" data-id="${horario.id}">
                            Eliminar
                        </button>
                    </td>
                </tr>
        ...
        this.querySelectorAll('.btn-ver-detalle').forEach(btn => {
            btn.addEventListener('click', () => {
                alert('Ver detalles del viaje ID: ' + btn.dataset.id);
            });
        });

        this.querySelectorAll('.btn-eliminar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm('¿Estás seguro de eliminar el viaje ' + btn.dataset.id + '?')) {
                    alert('Viaje eliminado: ' + btn.dataset.id);
                }
            });
        });
    }
...
customElements.define('runaway-tabla', RunawayTabla);